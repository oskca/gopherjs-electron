// Package dialog Displays native system dialogs for opening and saving files, alerting, etc.
// Process: Main
...
// If you want to use the dialog object from a renderer process, remember to access it using the remote:
//
...
// Sheets
//
// On macOS, dialogs are presented as sheets attached to a window if you provide a BrowserWindow reference in
// the browserWindow parameter, or modals if no window is provided.
//
// You can call BrowserWindow.getCurrentWindow().setSheetOffset(offset) to change the offset from
// the window frame where sheets are attached.
package dialog
...
// Get returns electron module dialog
func Get() *Dialogger {
	return &Dialogger{
		Object: electron.Get("dialog"),
	}
}

// Dialogger The dialog module has the following methods:
type Dialogger struct {
	*js.Object
	...
	ShowErrorBox func(title, content string) `js:"showErrorBox"`
}
...
// OptionOpen wraps the dialog creating options
type OptionOpen struct {
	...
	Filters     []FileFilter // filters FileFilter[] (optional)
...
const (
	...
	PropOpenDirectory   = "openDirectory"
	PropMultiSelections = "multiSelections"
	PropCreateDirectory = "createDirectory"
	PropShowHiddenFiles = "showHiddenFiles"
)
...
func (d *Dialogger) ShowOpenDialog(opt OptionOpen, bw ...*browserwindow.BrowserWindow) (filePaths []string) {
	var out *js.Object
	if len(bw) > 0 {
...
// OptionSave wraps for ShowSaveDialog
...
func (d *Dialogger) ShowSaveDialog(opt OptionSave, bw ...*browserwindow.BrowserWindow) (filepath string) {
	var out *js.Object
	if len(bw) > 0 {
...
type OptionMessage struct {
	...
	Icon      *nativeimage.NativeImage // icon NativeImage (optional)
...
func (o OptionMessage) toJs() js.M {
	m := make(js.M)
	if o.Type != "" {
		m["type"] = o.Type
	}
...
// ShowMessageBox dialog.showMessageBox([browserWindow, ]options[, callback])
// browserWindow BrowserWindow (optional)
// options Object
...
func (d *Dialogger) ShowMessageBox(opt OptionMessage, bw ...*browserwindow.BrowserWindow) (buttonIndex int) {
	var out *js.Object
	if len(bw) > 0 {
...
// ShowMessage easy form for ShowMessageBox
func (d *Dialogger) ShowMessage(title, msg string) {
	d.ShowMessageBox(OptionMessage{
		Title:   title,
		Type:    TypeInfo,
		Message: msg,
	})
